...
DEFAULT_LLM_ENDPOINT = os.environ.get("DYNAMIC_LLM_URL", "http://109.165.139.149:30406/v1/chat/completions")
...
# Prompt templates are loaded dynamically from prompt.txt in load_prompts()
...
def ensure_answers(answers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Normalize transport_layer_protocols
    protocols = _normalize_list(answers.get("transport_layer_protocols"))
    normalized["transport_layer_protocols"] = ", ".join(protocols)

    # Normalize documented_interfaces
    interfaces = _normalize_list(answers.get("documented_interfaces"))
    normalized["documented_interfaces"] = ", ".join(interfaces)

    # Normalize boolean is_dut_access_point for conditional logic
    normalized["is_dut_access_point_bool"] = _normalize_bool(answers.get("is_dut_access_point"))
...
def build_prompt(
    ...
    is_ap = user_answers.get("is_dut_access_point_bool", False)
    
    if is_ap:
        prompt_template = PROMPTS.get("COVERAGE_VALIDATOR_PROMPT_AP_DEVICES", "")
    else:
        prompt_template = PROMPTS.get("COVERAGE_VALIDATOR_PROMPT_NON_AP_DEVICES", "")
    ...
    payload = {
        ...
        "transport_layer_protocols": user_answers.get("transport_layer_protocols", ""),
        "documented_interfaces": user_answers.get("documented_interfaces", ""),
    }
...
def run_gap_analysis(
    ...
    is_ap = user_answers.get("is_dut_access_point_bool", False)
    if is_ap:
        prompt_template = PROMPTS.get("OUT_OF_SCOPE_VALIDATOR_PROMPT_AP_DEVICES", "")
    else:
        prompt_template = PROMPTS.get("OUT_OF_SCOPE_VALIDATOR_PROMPT_NON_AP_DEVICES", "")
    ...
    for idx, s in enumerate(scenarios, 1):
        ...
        prompt_text = render_prompt(prompt_template, render_values)
...
def _write_metadata_to_pipeline(user_answers: dict, pipeline_path: Path) -> None:
    """Write extra metadata (like ports/protocols) to the top-level of the pipeline output."""
    pipeline_path = pipeline_path.resolve()
    if pipeline_path.exists():
        try:
            data = json.loads(pipeline_path.read_text(encoding="utf-8").strip() or "{}")
    ...
    # Write documented_ports_protocols to the top level as a list
    data["documented_ports_protocols"] = _normalize_list(user_answers.get("documented_ports_protocols", ""))
    
    pipeline_path.write_text(json.dumps(data, indent=2), encoding="utf-8")
    print(f"Pipeline updated with metadata -> {pipeline_path.name}")
...
def run_validation(
    ...
    # Ensure metadata is written to pipeline output early
    _write_metadata_to_pipeline(user_answers, pipeline_path)
        ...
        # Fallback content checks
            ...
            try:
                ...
                if isinstance(data, dict):
                    if "sections" in data:
                        structured = path
                    elif "authentication_access_methods" in data or "transport_layer_protocols" in data:
                        answer = path
                    elif "gap_assessment" in data or "protocol_coverage" in data or "interface_protocol_coverage" in data:
                        pipeline = path
            except:
                pass
